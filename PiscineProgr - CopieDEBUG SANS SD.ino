...
const int cSizeSAction = sizeof(_sAction);
...
//  SDA   D18 A4 (23)    
//  SCL   D19 A5 (24)
// LiquidCrystal( rs, enable,d4, d5, d6, d7); coté shield LCD
// LiquidCrystal lcd(12, 11, 5, 4, 3, 2);     coté LCD sans shield
...
//---------------------------------------------------------------------------------
/*
char* VersDegre(int n, char res[]) {
    char c, cRef;
    int i = 0;

    for (i = 0; i < lgNOMPROG + 1; i++) {
        c = tRemote[n].nomprog[i];
        if (c == 0) break;
        res[i] = c;
        cRef = c;
    }
        ...
        if ( FlagMinute ) {
          ...
          // chaque mn  mise en tableau de la temperature 60 valeurs
            ...
            Message = " H="
                ...
                + String(iActionH, DEC)
                + String(") mn=")
                + String(gTm.Minute, DEC)
                + String(" (M=")
                + String(bActionMn)
                + String(") Tpr=")
                + String(tprMn)
                + String(" Pg=")
                + String(gT_Action.s.nProg);
...
void setup(void) {
  ...
  if ( !(  (gT_Action.s.test[0] == 'J')
      ...
      SauveDansRTC();  // adresse 0  sauve nProg
#ifdef DEBUG
      Message = "Déf " + String(gT_Action.s.test);
      Serial.println(Message);
#endif
